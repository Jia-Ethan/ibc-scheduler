...
export async function saveUserProfile(profile: UserProfile): Promise<void> {
  ...
  console.warn('Extended profile sync failed, retrying phone-only sync:', error);
  ...
  const { error: phoneError } = await supabase
    .from('user_profiles')
    .upsert([phoneOnlyPayload], { onConflict: 'id' });

  if (phoneError) {
    console.error('Error saving user profile:', phoneError);
    throw phoneError;
  }
}
