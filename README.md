...
當前 Word 導出模板以 `排班表_YYYY-MM-DD.docx` 命名，內容口徑固定為：

- 標題：`国际商学院学生助理值班表`
- 日期：`更新日期：YYYY年M月D日`
- 主表：第一列為 `節次`，後續列為 `周一` 至 `周五`，行為 `第1節` 至 `第8節`
- 值班格：只顯示值班人姓名；空班格保持空白
- 聯繫電話：表格下方顯示 `值班人员联系电话`，格式為 `姓名：电话`

聯繫電話優先讀取 `user_profiles.phone`，並會合併系統內置的學生資料種子；若某位同學電話缺失，Word 中顯示 `未设置`，不會阻斷導出。
